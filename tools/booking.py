"""
Booking Tools for Cinema Agent
...
def process_payment(booking_id: str,
                   ...
                   card_last_four: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        card_last_four: Last 4 digits of card (for simulation)
...
@tool
def cancel_booking(booking_id: str, reason: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    # Check cancellation policy (simulated - 2 hours before showtime)
    showtime = datetime.fromisoformat(booking["showtime"].replace(" ", "T"))
    hours_until_show = (showtime - datetime.now()).total_seconds() / 3600
    
    if hours_until_show < 2:
        return {"error": "Cannot cancel booking less than 2 hours before showtime"}
    
    # Process cancellation
    booking["status"] = "cancelled"
    booking["cancelled_at"] = datetime.now().isoformat()
    booking["cancellation_reason"] = reason or "Customer requested"
    
    refund_amount = 0
    if booking.get("payment"):
        # Full refund if more than 24 hours, 50% if less
        if hours_until_show > 24:
            refund_amount = booking["pricing"]["total"]
        else:
            refund_amount = booking["pricing"]["total"] * 0.5
    ...
    return {
        ...
        "refund": {
            ...
            "status": "processing" if refund_amount > 0 else "not_applicable",
            "estimated_days": 3 if refund_amount > 0 else 0
        },
        "message": f"Booking cancelled successfully. {'Refund will be processed within 3-5 business days.' if refund_amount > 0 else 'No refund applicable for unpaid bookings.'}"
    }