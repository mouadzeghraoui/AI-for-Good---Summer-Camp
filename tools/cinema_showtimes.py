"""
Cinema and Showtime Tools using MovieGlu API
Tools for finding cinemas, checking showtimes, and seat availability
"""
...
def get_movieglu_headers(endpoint: str) -> Dict[str, str]:
    ...
    # MovieGlu requires specific headers including a signature
    headers = {
        "client": MOVIEGLU_CLIENT,
        "x-api-key": MOVIEGLU_API_KEY,
        "authorization": MOVIEGLU_AUTHORIZATION,
        "territory": os.getenv('MOVIEGLU_TERRITORY', 'GB'),  # Default to UK
        "api-version": "v200",
        "geolocation": os.getenv('MOVIEGLU_GEOLOCATION', '51.5074;-0.1278'),  # Default to London
...
@tool
def find_cinemas_nearby(latitude: float = 51.5074, 
    ...
    Args:
        latitude: Latitude of the location (default: London)
        longitude: Longitude of the location (default: London)
...
def get_cinema_showtimes(cinema_id: str, 
    ...
    Args:
        ...
        movie_id: Optional TMDb movie ID to filter showtimes
    ...
    try:
        ...
        for film in films:
            ...
            if not movie_id or str(film.get('film_id')) == movie_id:
...
@tool
def check_film_availability(film_id: str,
                           date: Optional[str] = None,
                           latitude: float = 51.5074,
                           longitude: float = -0.1278) -> Dict[str, Any]:
    """
...
@tool
def get_film_info_by_title(title: str) -> Dict[str, Any]:
    """
